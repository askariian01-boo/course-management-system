    ...
    <div class="card p-4 bg-white rounded-3 mt-3 ml-3 mr-3">
        ...
        <form method="GET" action="" class="row g-2 mb-3">
            ...
            <div class="col-12 col-md-6 d-flex align-items-center gap-2">
                ...
                <!-- Add Attendance -->
                @if (Auth::user()->can('income_add'))
                    <a href="{{ route('income_add') }}" class="btn btn-sm btn-primary"
                        style="padding-top: 0.65rem; padding-bottom: 0.6rem; font-weight: 600;">
                        <i class="fa fa-add"></i> add income
                    </a>
                @endif
                @if (Auth::user()->can('income_source_list'))
                    <a href="{{ route('income_source_list') }}" class="btn btn-sm btn-primary"
                        style="padding-top: 0.65rem; padding-bottom: 0.6rem; font-weight: 600;">
                        <i class="fa fa-eye"></i> income source
                    </a>
                @endif
            </div>
        </form>
                ...
                <tbody>
                    @forelse($incomes as $income)
                        <tr>
                            ...
                            <td>
                                <p>{{ $income->IncomeSource->source_name }}</p>
                            </td>
                            <td>
                                <p>{{ $income->income_amount }} AFG</p>
                            </td>
                            <td>
                            ...
                            @if (Auth::user()->can('income_edit'))
                                <td>
                                    <a href="{{ route('income_edit', $income->id) }}" class="btn btn-sm btn-primary"><i
                            ...
                            @if (Auth::user()->can('income_delete'))
                                <td>
                                    <p>
                                    <form action="{{ route('income_delete', $income->id) }}" method="POST"
                                        class="d-inline">
                                        @csrf
                                        @method('DELETE')
