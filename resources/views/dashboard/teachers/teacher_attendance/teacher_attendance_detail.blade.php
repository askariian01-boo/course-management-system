...
@section('content')
    <div class="card-body mt-3 ml-4 mr-4 p-4 bg-white rounded">
        ...
        <h4 class="mb-4 fw-bold text-primary">
            Attendance Calendar - {{ $teacher->FirstName }} {{ $teacher->LastName }}
        </h4>
        ...
        <form method="GET" action="" class="row g-2 mb-3">
            ...
            <div class="col-12 col-md-4 d-flex align-items-center gap-2">
                ...
                <!-- Add Attendance -->
                <a href="{{ route('teacher_attendance_list') }}" class="btn btn-info flex-fill text-white"
                    style="padding-top: 0.65rem; padding-bottom: 0.6rem;">
            ...
            @for ($day = 1; $day <= $daysInMonth; $day++)
                ...
                @elseif($status == 'present') present @endif">
                    ...
                    <div class="calendar-actions">
                        ...
                        @if ($status == 'absent')
                            {{-- Edit --}}
                            @if (Auth::user()->can('teacher_attendance_edit'))
                                <a href="" class="btn btn-primary btn-sm">
                                    <i class="fa fa-edit"></i>
                                </a>
                            @endif
                            @if (Auth::user()->can('teacher_attendance_delete'))
                                {{-- Delete --}}
                                <form action="{{ route('teacher_attendance_delete', [$teacher->id, $date]) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
