...
@section('content')
    <div class="col-lg-12 mt-3">
        <div class="card-style">
            <h4 class="mb-10"> Teacher Documents
                @if (Auth::user()->can('teacher_document_add'))
                    <a href="{{ route('teacher_document_add') }}" class="btn btn-sm btn-info float-right">Add New
                        Document</a>
                @endif
            </h4><br>
            <div class="table-wrapper table-bordered table-responsive">
                <table class="table table-bordered table-striped table-hover tableD" id="myTable">
                    <thead class="text-center">
                        <tr>
                            <th>
                                <h6 style="font-weight: bold;">teacher</h6>
                            </th>
                            <th>
                            ...
                            @if (Auth::user()->can('teacher_document_edit'))
                                <th>
                                    <h6 style="font-weight: bold;">edit</h6>
                                </th>
                            @endif
                            @if (Auth::user()->can('teacher_document_delete'))
                                <th>
                    ...
                    <tbody class="text-center">
                        @foreach ($documents as $document)
                            <tr>
                                <td class="min-width">
                                    <p>{{ $document->Teacher->FirstName }} {{ $document->Teacher->LastName }}
                                    </p>
                                </td>
                                <td class="min-width">
                                ...
                                <td class="text-center">
                                    ...
                                    <a class="btn btn-sm btn-primary" style="margin-top:-15px;"
                                        href="{{ route('teacher_documents_download', $document->document_file) }}"
                                ...
                                @if (Auth::user()->can('teacher_document_edit'))
                                    <td>
                                        <a href="{{ route('teacher_document_edit', $document->id) }} "style="margin-top:-15px;"
                                ...
                                @if (Auth::user()->can('teacher_document_delete'))
                                    <td>
                                        <form
                                            action="{{ route('teacher_document_delete', $document->id) }}"style="margin-top:-15px;"
