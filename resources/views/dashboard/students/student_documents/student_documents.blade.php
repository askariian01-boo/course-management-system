...
@section('content')
    <div class="col-lg-12 mt-3">
        <div class="card-style">
            <h4 class="mb-10"> Student Documents
                @if (Auth::user()->can('student_document_add'))
                    <a href="{{ route('student_document_add') }}" class="btn btn-sm btn-primary float-right"><i class="fa fa-add"></i>add new
                        document</a>
                @endif
            </h4><br>
            <div class="table-wrapper table-bordered table-responsive">
                <table class="table table-striped table-hover table-bordered tableD" id="myTable">
                    <thead class="text-center thead-light">
                        <tr>
                            <th>
                                <h6 style="font-weight: bold;">student</h6>
                            </th>
                            <th>
                            ...
                            @if (Auth::user()->can('student_document_edit'))
                                <th>
                                    <h6 style="font-weight: bold;">edit</h6>
                                </th>
                            @endif
                            @if (Auth::user()->can('student_document_delete'))
                                <th>
                    ...
                    <tbody class="text-center">
                        @foreach ($documents as $document)
                            <tr>
                                <td class="min-width">
                                    <p>{{ $document->student->FirstName }} {{ $document->student->LastName }}
                                    </p>
                                </td>
                                <td class="min-width">
                                ...
                                <td class="text-center">
                                    ...
                                    <a class="btn btn-sm btn-primary" style="margin-top:-15px;"
                                        href="{{ route('student_document_download', $document->document_file) }}"
                                ...
                                @if (Auth::user()->can('student_document_edit'))
                                    <td>
                                        <a href="{{ route('student_document_edit', $document->id) }} "style="margin-top:-15px;"
                                ...
                                @if (Auth::user()->can('student_document_delete'))
                                    <td>
                                        <form
                                            action="{{ route('student_document_delete', $document->id) }}"style="margin-top:-15px;"
