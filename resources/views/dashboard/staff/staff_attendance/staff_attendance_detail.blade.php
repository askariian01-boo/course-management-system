...
@section('content')
    <div class="card-body mt-3 ml-4 mr-4 p-4 bg-white rounded">
        ...
        <h4 class="mb-4 fw-bold text-primary">
            Attendance Calendar - {{ $staff->FirstName }} {{ $staff->LastName }}
        </h4>
        ...
        <form method="GET" action="" class="row g-2 mb-3">
            ...
            <div class="col-12 col-md-4 d-flex align-items-center gap-2">
                ...
                <!-- Add Attendance -->
                <a href="{{ route('staff_attendance_list') }}" class="btn btn-primary flex-fill text-white fw-bold"
                    style="padding-top: 0.65rem; padding-bottom: 0.6rem;">
            ...
            @for ($day = 1; $day <= $daysInMonth; $day++)
                ...
                @elseif($status == 'present') present @endif">
                    ...
                    <div class="calendar-actions">
                        ...
                        @if ($status == 'absent')
                            {{-- Edit --}}
                            @if (Auth::user()->can('employee_attendance_edit'))
                            ...
                            @if (Auth::user()->can('employee_attendance_delete'))
                                <form action="{{ route('staff_attendance_delete', [$staff->id, $date]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
